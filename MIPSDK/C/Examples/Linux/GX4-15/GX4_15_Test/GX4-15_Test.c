...
int main(int argc, char* argv[])
{
 ...
 if(com_mode == MIP_SDK_GX4_15_IMU_STANDARD_MODE)
 {
  ...
  //Request basic status report
  while(mip_3dm_cmd_hw_specific_device_status(&device_interface, GX4_15_MODEL_NUMBER, GX4_15_BASIC_STATUS_SEL, &basic_field) != MIP_INTERFACE_OK){} 
  ...
  //Request diagnostic status report
  while(mip_3dm_cmd_hw_specific_device_status(&device_interface, GX4_15_MODEL_NUMBER, GX4_15_DIAGNOSTICS_STATUS_SEL, &diagnostic_field) != MIP_INTERFACE_OK){} 

  printf("Model Number: \t\t\t\t\t%04u\n", diagnostic_field.device_model);
  printf("Status Selector: \t\t\t\t%s\n", diagnostic_field.status_selector == GX4_15_BASIC_STATUS_SEL ? "Basic Status Report" : "Diagnostic Status Report");
  ...
  while(mip_filter_reference_position(&device_interface, MIP_FUNCTION_SELECTOR_WRITE, &reference_position_enable_command, reference_position_command) != MIP_INTERFACE_OK){}
  ...
  while(mip_filter_reference_position(&device_interface, MIP_FUNCTION_SELECTOR_LOAD_DEFAULT, NULL, NULL) != MIP_INTERFACE_OK){}
